...
@BLACS_tab
class PicomotorsTab(DeviceTab):
    def initialise_GUI(self):
        layout = self.get_tab_layout()
        ui_filepath = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "camera.ui"
        )
        ...
        port = int(
            ...
            .find_by_name(self.settings["device_name"])
            .BLACS_connection
        )
        self.ui.port_label.setText(str(port))

        self.ui.check_connectivity_pushButton.setIcon(
            QIcon(":/qtutils/fugue/arrow-circle")
        )

        self.ui.host_lineEdit.returnPressed.connect(
            self.update_settings_and_check_connectivity
        )
        self.ui.use_zmq_checkBox.toggled.connect(
            self.update_settings_and_check_connectivity
        )
        self.ui.check_connectivity_pushButton.clicked.connect(
            self.update_settings_and_check_connectivity
        )
    ...
    def initialise_workers(self):
        ...
        self.create_worker(
            "main_worker", PicomotorsWorker, worker_initialisation_kwargs
        )
    ...
    def update_settings_and_check_connectivity(self, *args):
        ...
        responding = yield (
            self.queue_work(
                self.primary_worker, "update_settings_and_check_connectivity", **kwargs
            )
        )
