...
class RigolScope(Device):
    description = 'Interface for rigol scopes'
    @set_passed_properties(
        property_names = {
            "connection_table_properties": ["BIAS_port"],
        }
    )
    def __init__(self, name,  BIAS_port, scope_name, **kwargs):
    ...
    def generate_code(self, hdf5_file):
        ...
        group.attrs["scope_name"] = self.scope_name
        group.attrs["timestep"] = self.timestep
        group.attrs["offset"] = self.offset
        try:
            names, channels = zip(*self.traces.items())
        except ValueError:
            names, channels = [], []
            print("No Rigol Scope trace")
        group.attrs["channels"] = channels
        group.attrs["names"] = names

    def acquire(self, name, channel):
        if channel in self.traces.values():
            raise Exception(f"Channel {channel} already has trace assigned")
        self.traces[name] = channel
...
@BLACS_tab
class RigolScopeTab(DeviceTab):
    def initialise_GUI(self):
        layout = self.get_tab_layout()
        ui_filepath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'device.ui')
    ...
    def initialise_workers(self):
        ...
        self.create_worker("main_worker", RigolScopeWorker, worker_initialisation_kwargs)
